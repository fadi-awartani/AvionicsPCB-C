...
//Based on flowchart from microcontroller datasheet.
int read_i2c_bytes_no_addr(uint8_t device_address, uint8_t *data_bytes, int len) {
	int count = 0, i;
	setup_i2c_masterclock();
	AVR32_TWI.cr = 0x24;
	AVR32_TWI.mmr = (device_address << 16) | 0x1000;
	if(len > 1) {
		AVR32_TWI.cr = 0x1; //Start transfer
		...
		for(i = 0; i < len-1; i++) {
			...
			while(!((status_reg = AVR32_TWI.sr) & 0x2)) { //While not RX ready - busy wait loop
				if(status_reg & 0x300) //if bits of ARBLST, NACK are set.
					return -1; //error
					
				if(status_reg & 1) //if TXCOMP is set
					break;
